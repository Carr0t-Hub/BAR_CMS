        ...
        <table id="dataTable" class="table table-bordered table-hover table-striped">
          ...
          <tbody>
            <?php foreach ($res as $key => $value) : ?>
              <tr>
                <td><?php echo strtoupper($value['agencyName']); ?></td>
                ...
                <td>
                  <div class="d-grid gap-2">
                    <button class="btn btn-primary attachedItem" type="button" name="editData" id="editData" data-id="<?= $value['id'] ?>"><i class="ri-edit-2-line"></i> Edit</button>
                  </div>
                </td>
              </tr>
            <?php endforeach; ?>
...
<script>
  $(document).ready(function() {
    $('.attachedItem').click(function() {
      var id = $(this).attr('data-id');

      $.ajax({
        url: 'editAttached.php',
        type: 'POST',
        data: {
          id: id
        },
